...
class Network(object):
    ...
    @layer
    def field_embed(self, input, limits, name, embed_size = 8, trainable = True):
        with tf.variable_scope(name) as scope:
            features = input[0]
            vals = input[1]
            features_size = limits[-1]
            field_size = len(limits) -1 

            init_weights = tf.truncated_normal_initializer(0.0, stddev=0.001)
            init_biases = tf.constant_initializer(0.0)

            weights = self.make_var('weights', [features_size, field_size, embed_size], init_weights, trainable)
