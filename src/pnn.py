...
class Pnn(object):
    def __init__(self, limits, embed_size=8, l2_reg_lambda=0.0, NUM_CLASSES=2 ):
        ...
        ###############product_layer################
        self.pl_output = product_layer_op(tf.tanh(self.fe_output))
#r       self.relu_output = tf.nn.relu(self.pl_output)
        self.relu_output = tf.tanh(self.pl_output)
        ###############fc1#########################
        '''
        fe_shape = self.fe_output.shape
        fe_size = fe_shape[1]*fe_shape[2]*fe_shape[3]
        print(self.fe_output.shape)
        ...
        self.fc1_weights = tf.get_variable('fc1_weights', [dim, 10], None, init_weights)
        self.fc1_biases = tf.get_variable('fc1_biases', [10], None, init_biases)

        self.relu_output = tf.nn.relu(tf.matmul(self.concat_output, self.fc1_weights) + self.fc1_biases)
        '''
        ##############fc#######################
        ...
        losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits, labels=self.labels_ph)
